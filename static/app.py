...
def medals_query():
    print ("query started")
    import pandas as pd
    medals_info = pd.read_sql('SELECT year, country_code, SUM (medal_value) as totalmedals FROM Sports GROUP BY year, country_code', conn)
    medals_list =  medals_info.to_dict('records')
